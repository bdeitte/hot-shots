...
* [@bdeitte](https://github.com/bdeitte) Improve `cacheDns` behavior: concurrent sends share a single in-flight lookup, a send past the TTL goes out immediately on the last known address while one refresh runs in the background, and a failed lookup waits a full TTL before the next attempt instead of retrying on every send
