...
* [@bdeitte](https://github.com/bdeitte) Bound the queue of sends waiting on an in-flight `cacheDns` lookup to 1000. A hung resolver under a high send rate could otherwise grow this queue without limit; past the cap, the oldest queued send is dropped and its callback receives an error. With `includeDatadogTelemetry` enabled this is now counted as `packets_dropped_queue` (backpressure), not `packets_dropped_writer` (a write/transport failure), matching the Datadog DogStatsD telemetry schema. The cap is now enforced by pushing first and trimming back down to the limit before invoking any dropped callback, so a drop callback that synchronously sends again (e.g. an errorHandler emitting a metric) can no longer push the queue above the cap. Those dropped callbacks are invoked on a deferred tick rather than synchronously, so a resend from within a drop callback starts a fresh call stack instead of recursing.
