...
* [@bdeitte](https://github.com/bdeitte) Improve `cacheDns` behavior:
     ...
     * hot-shots pins lookups to the socket's address family. A `udp4` client resolving a hostname could previously get an IPv6 address back and fail every send with `EINVAL`, which `localhost` reproduces on most modern systems
...
* [@bdeitte](https://github.com/bdeitte) Bound and contain failures on the send path:
     * Refuse `tcp` and `stream` sends once 1 MiB is waiting to flush, since Node otherwise queues writes in memory without limit while a socket is connecting or its peer has stalled. Refused sends fail with `HOTSHOTS_WRITE_QUEUE_FULL` and, with `includeDatadogTelemetry`, count as `packets_dropped_queue`
     ...
     * Contain a user callback or `errorHandler` that throws. Every path that calls back a batch of sends now reports the throw with `console.error` and continues, so one bad callback can no longer abandon the rest of the batch or, from `close()`, leave the socket open and the close callback never fired
