...
* [@bdeitte](https://github.com/bdeitte) Bound the memory a stalled or unreachable receiver can retain. `tcp` and `stream` sends are refused once 1 MiB is waiting to flush, since Node otherwise queues writes without limit while a socket is connecting or its peer has stalled. The `uds` sends waiting out a retry backoff are capped at 1000, dropping the oldest past that; `udsRetryOptions.retries` caps the attempts for one send, not how many sends retry at once
* [@bdeitte](https://github.com/bdeitte) Give every refused send a `HOTSHOTS_*` error code and count it as `packets_dropped_queue` rather than a writer error, and add the matching `bytes_dropped_queue` to the Datadog telemetry. NETWORKING.md lists each code and its telemetry bucket
...
* [@bdeitte](https://github.com/bdeitte) Dev-only: add a Docker perfTest harness that reports suite wall time and network, DNS and syscall counts (see `perfTest/README.md`), test Node.js 26 in CI, and refresh the lockfile for Node.js 26 and open dev-dependency advisories
