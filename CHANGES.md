...
* [@bdeitte](https://github.com/bdeitte) BREAKING: Drop Node.js 18 support, now requires Node.js >= 20.0.0. The TCP transport relies on `autoSelectFamily`, which only exists in later Node 18 patch releases, and Node 18 reached end of life in April 2025
...
* [@bdeitte](https://github.com/bdeitte) Improve `cacheDns` behavior:
     * Concurrent sends share a single in-flight lookup, and a send past the TTL goes out immediately on the last known address while one refresh runs in the background
     * A failed lookup waits before the next attempt instead of retrying on every send — one second after the first failure, doubling with each consecutive failure, capped at `cacheDnsTtl` — so a resolver briefly unavailable at startup costs about a second of metrics rather than a full TTL
     * Lookups are constrained to the socket's address family. A `udp4` client resolving a hostname could previously get an IPv6 address back and fail every send with `EINVAL`, which `localhost` reproduces on most modern systems
...
* [@bdeitte](https://github.com/bdeitte) Default the TCP host to `127.0.0.1` when none is given, extending to TCP the same loopback handling UDP already had ([#185](https://github.com/bdeitte/hot-shots/issues/185)). A TCP client with no `host` previously connected to `localhost`, which costs a DNS lookup and resolves to `::1` before `127.0.0.1` on most systems, so it could not reach an agent listening on IPv4 only. TCP connections to an explicit hostname now also try every resolved address family rather than only the first, so `autoSelectFamily` still applies when Node's default for it is turned off ([#222](https://github.com/bdeitte/hot-shots/issues/222)). Pass `host: '::1'` to reach an IPv6 agent
...
* [@bdeitte](https://github.com/bdeitte) Bound and contain failures on the send path:
     * Refuse `tcp` and `stream` sends once 1 MB is waiting to flush, since Node otherwise queues writes in memory without limit while a socket is connecting or its peer has stalled. Refused sends fail with `HOTSHOTS_WRITE_QUEUE_FULL` and, with `includeDatadogTelemetry`, count as `packets_dropped_queue`
     * Deliver send failures on a later tick rather than on the same stack frame as the send, so an `errorHandler` that emits a metric on failure can no longer grow the stack without bound against a persistently failing transport
     * Contain a user callback or `errorHandler` that throws. Every path that calls back a batch of sends now reports the throw with `console.error` and continues, so one bad callback can no longer strand the rest of the batch or, from `close()`, leave the socket open and the close callback never fired
     * Add `bytes_dropped_queue` to the Datadog telemetry, alongside the existing `packets_dropped_queue` and the `_writer` pair
* [@bdeitte](https://github.com/bdeitte) Add NETWORKING.md, documenting the send path and failure modes of each transport with diagrams, linked from README.md, and a perfTest Docker harness that runs the full suite on Ubuntu and reports suite wall time, network and DNS call counts, and syscall counts (see `perfTest/README.md`)
