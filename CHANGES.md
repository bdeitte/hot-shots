...
* [@bdeitte](https://github.com/bdeitte) Make `close()` complete instead of hanging. The final flush is bounded at 5 seconds, a stalled `cacheDns` lookup, an incomplete `tcp` connect, an already-destroyed socket and a waiting `uds` retry no longer block it, and every queued send's callback fires exactly once
