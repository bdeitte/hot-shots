...
* [@bdeitte](https://github.com/bdeitte) BREAKING: Drop Node.js 18 support, now requires Node.js >= 20.0.0. The TCP transport relies on `autoSelectFamily`, which only exists in later Node 18 patch releases. Node 18 also reached end of life in April 2025
* [@bdeitte](https://github.com/bdeitte) Eliminate per-packet DNS lookups on UDP. Node routes every packet's destination through the socket's `lookup`, so hot-shots now short-circuits IP literals there. A client with an IP `host`, or no `host` at all, makes no `dns.lookup` calls and creates none of the APM spans they generate. Reported against dd-trace-js as [DataDog/dd-trace-js#2984](https://github.com/DataDog/dd-trace-js/issues/2984)
* [@bdeitte](https://github.com/bdeitte) Improve `cacheDns` behavior:
     * Concurrent sends share a single in-flight lookup, and a send past the TTL goes out immediately on the stale address while one refresh runs in the background
     * A failed lookup waits before the next attempt instead of retrying on every send: one second after the first failure, doubling with each consecutive failure, capped at `cacheDnsTtl`. A resolver briefly unavailable at startup therefore costs about a second of metrics rather than a full TTL
     * hot-shots pins lookups to the socket's address family. A `udp4` client resolving a hostname could previously get an IPv6 address back and fail every send with `EINVAL`. `localhost` reproduces this on most modern systems
     * The queue of sends waiting on a cold-start lookup is bounded at 1000, dropping the oldest past that. Dropped sends fail their callback and, with `includeDatadogTelemetry`, count as `packets_dropped_queue`
* [@bdeitte](https://github.com/bdeitte) BREAKING: Default the TCP host to `127.0.0.1` when none is given, extending to TCP the same loopback handling UDP already had ([#185](https://github.com/bdeitte/hot-shots/issues/185)). A TCP client with no `host` previously connected to `localhost`, which costs a DNS lookup and resolves to `::1` before `127.0.0.1` on most systems. It could therefore not reach an agent listening on IPv4 only. A TCP client with no `host` whose agent listens on `::1` only, or whose `localhost` maps elsewhere, stops delivering on upgrade until it is given an explicit `host`. TCP connections to an explicit hostname now also try every resolved address family rather than only the first, so `autoSelectFamily` still applies when Node's default for it is disabled ([#222](https://github.com/bdeitte/hot-shots/issues/222)). Pass `host: '::1'` to reach an IPv6 agent
* [@bdeitte](https://github.com/bdeitte) Make `close()` complete instead of hanging in several cases:
     * The final buffered flush is bounded at 5 seconds on every transport, not just for a stalled `cacheDns` lookup. A `tcp` client whose connection never completes previously hung `close()` forever
     * A stalled `cacheDns` lookup no longer blocks buffered or unbuffered closes. Every queued send's callback fires exactly once, and sends issued after close fail fast rather than queueing
     * A socket that was already destroyed, reachable when an application destroys the `stream` it supplied and then closes the client, no longer hangs the close
     * A `uds` retry still waiting out its backoff is abandoned rather than fired against a closing socket up to `maxRetryDelayMs` later, holding the event loop open until then. Those sends fail with `HOTSHOTS_UDS_RETRY_CANCELLED`
* [@bdeitte](https://github.com/bdeitte) Bound and contain failures on the send path:
     * Refuse `tcp` and `stream` sends once 1 MiB is waiting to flush. Node otherwise queues writes in memory without limit while a socket is connecting or its peer has stalled. Refused sends fail with `HOTSHOTS_WRITE_QUEUE_FULL` and, with `includeDatadogTelemetry`, count as `packets_dropped_queue`
     * Cap the `uds` sends waiting out a retry backoff at 1000, dropping the oldest past that. `udsRetryOptions.retries` caps the attempts for one send, not how many sends are retrying at once, so a receiver stalled on `EAGAIN` previously retained every flush that arrived during the stall. Dropped sends fail with `HOTSHOTS_UDS_RETRY_QUEUE_FULL` and, with `includeDatadogTelemetry`, count as `packets_dropped_queue`
     * Deliver send failures on a later tick rather than on the same stack frame as the send. An `errorHandler` that emits a metric on failure can therefore no longer grow the stack without bound against a persistently failing transport
     * Contain a user callback or `errorHandler` that throws. Every path that calls back a batch of sends now reports the throw with `console.error` and continues. One bad callback can therefore no longer abandon the rest of the batch, or leave `close()` with an open socket and a callback that never fires. A throwing `errorHandler` is now contained on every path that reaches it, including the socket `error` event, the single-send failure, a failed socket creation or replacement, a send refused after `close()`, and `check()`/`event()` under `telegraf`. Most of those calls run inside an event emit, a timer, or the transport's write callback, so the throw previously escaped as an uncaught exception and ended the process. The `telegraf` ones instead surfaced a handler bug at an unrelated call site
