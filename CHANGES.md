...
* [@bdeitte](https://github.com/bdeitte) Fix TCP tests hanging or timing out on Linux hosts where `localhost` resolves to `::1` before `127.0.0.1`: the test servers in `test/helpers/helpers.js` and the inline server in `test/transport.js` now bind `127.0.0.1` explicitly instead of `localhost`, so the server and the client agree on the same address family
...
* [@bdeitte](https://github.com/bdeitte) Improve `cacheDns` behavior: concurrent sends share a single in-flight lookup, a send past the TTL goes out immediately on the last known address while one refresh runs in the background, and a failed lookup waits before the next attempt instead of retrying on every send. That wait is one second after the first failure and doubles with each consecutive failure, capped at `cacheDnsTtl`, so a resolver that is briefly unavailable at startup costs about a second of metrics rather than a full TTL
* [@bdeitte](https://github.com/bdeitte) Constrain `cacheDns` lookups to the socket's address family. A `udp4` client resolving a hostname could previously get an IPv6 address back and fail every send with `EINVAL`, which `localhost` reproduces on most modern systems
* [@bdeitte](https://github.com/bdeitte) Bound the `cacheDns` queue of sends waiting on a cold-start lookup at 1000, dropping the oldest past that. Dropped sends fail their callback and, with `includeDatadogTelemetry`, count as `packets_dropped_queue`
...
* [@bdeitte](https://github.com/bdeitte) Refuse `tcp` and `stream` sends once 1 MB is waiting to flush, since Node otherwise queues writes in memory without limit while a socket is connecting or its peer has stalled. Previously these writes were buffered in memory without limit until the peer drained. Refused sends fail with `HOTSHOTS_WRITE_QUEUE_FULL` and, with `includeDatadogTelemetry` enabled, count as `packets_dropped_queue`
...
* [@bdeitte](https://github.com/bdeitte) Contain a user callback or `errorHandler` that throws. Every path that calls back a batch of sends now reports the throw with `console.error` and continues, so one bad callback can no longer strand the rest of the batch or, from `close()`, leave the socket open and the close callback never fired
* [@bdeitte](https://github.com/bdeitte) Abandon a `uds` retry still waiting out its backoff when `close()` runs, instead of firing it against a closing socket up to `maxRetryDelayMs` later and holding the event loop open until then. Those sends now fail with `HOTSHOTS_UDS_RETRY_CANCELLED`
* [@bdeitte](https://github.com/bdeitte) Add `bytes_dropped_queue` to the Datadog telemetry, alongside the existing `packets_dropped_queue` and the `_writer` pair
