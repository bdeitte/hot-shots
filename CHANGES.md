...
* [@bdeitte](https://github.com/bdeitte) Bound three failure modes that affected every transport, not just UDP. A send failure is now always delivered on a later tick rather than on the same stack frame as the send, so the documented "emit a metric on send failure" `errorHandler` pattern can no longer grow the stack without bound against a persistently failing transport (a destroyed `tcp` socket or `stream`, or a closed UDP socket, previously grew it by a constant per resend until the process wedged). `close()` now bounds the final buffered flush for every transport instead of only for a stalled `cacheDns` lookup: a `tcp` client whose connect never completes used to hang `close()` forever, and now closes the socket and invokes its callback after the same 5 second budget, reporting the dropped flush. `tcp` and `stream` transports refuse further sends once 1 MB is waiting to flush, since Node queues writes in memory without limit while a socket is connecting or its peer has stopped reading; refused sends fail with `HOTSHOTS_WRITE_QUEUE_FULL` and count as `packets_dropped_queue`. Relatedly, `close()` no longer hangs when the socket was already destroyed before it ran — reachable when an application destroys the `stream` it supplied and then closes the client — since `destroy()` is a no-op on an already-destroyed socket and emits no second `close` event for `close()` to wait on.

* [@bdeitte](https://github.com/bdeitte) Eliminate per-packet DNS lookups on UDP. Node routes every packet's destination address through the socket's `lookup`, so hot-shots now short-circuits IP literals there: a client with an IP `host`, or no `host` at all, performs no `dns.lookup` calls at all, and creates none of the APM spans they generate. With `cacheDns` and a hostname, concurrent sends share a single in-flight lookup, a send past the TTL goes out immediately on the last known address while one background refresh runs, and a failed lookup — on a cold start as well as on a refresh — earns a full TTL of cooldown before the next attempt, so a fast-failing resolver does not degrade back to a lookup per metric. The queue of sends waiting on a cold-start lookup is bounded at 1000; past that the oldest is dropped, its callback receives an error, and with `includeDatadogTelemetry` enabled it is counted as `packets_dropped_queue` (as is a send refused because the client is closed). `close()` now behaves correctly when a lookup is stalled: buffered and unbuffered closes both complete, every queued send's callback fires exactly once, and sends issued after close fail fast rather than queueing (with an accurate error message and its own error code, since a `cacheDns` client rejects post-close sends whether or not a lookup was actually stalled). `close()` gives an in-flight cold-start lookup up to 5 seconds to resolve before giving up on the final buffered flush, so an ordinary slow first lookup (e.g. the serverless "buffer then close immediately" shape) doesn't lose it. Reported against dd-trace-js as [DataDog/dd-trace-js#2984](https://github.com/DataDog/dd-trace-js/issues/2984).
