...
* [@bdeitte](https://github.com/bdeitte) BREAKING: Drop Node.js 18 support, now requires Node.js >= 20.0.0. Node 18 reached end of life in April 2025, and the TCP change below relies on `autoSelectFamily`, which the earliest Node 18 releases lack
* [@bdeitte](https://github.com/bdeitte) BREAKING: Default the `tcp` host to `127.0.0.1` when none is given, extending to TCP the loopback handling UDP already had ([#185](https://github.com/bdeitte/hot-shots/issues/185)). A `tcp` client with no `host` previously connected to `localhost`, which costs a DNS lookup and resolves to `::1` before `127.0.0.1` on most systems, so it could not reach an agent listening on IPv4 only. Such a client whose agent listens on `::1` only, or whose `localhost` maps elsewhere, stops delivering on upgrade until it is given an explicit `host`. TCP connections to an explicit hostname now also try every resolved address family rather than only the first. See ([#222](https://github.com/bdeitte/hot-shots/issues/222))
...
* [@bdeitte](https://github.com/bdeitte) Improve `cacheDns` behavior:
     ...
     * A failed lookup waits before the next attempt instead of retrying on every send: one second after the first failure, doubling with each consecutive failure, capped at five seconds. Only an incoming send retries the lookup, so that wait is also how long the client keeps refusing after the resolver recovers, which is why the ceiling is seconds rather than a TTL
     * A send refused during that wait keeps the resolver's own error code, such as `ENOTFOUND`, so an existing `errorHandler` keeps matching through a failure streak. The refusal is marked with `hotShotsCode` set to `HOTSHOTS_DNS_COOLDOWN`
     * Lookups are pinned to the socket's address family. A `udp4` client resolving a hostname could previously get an IPv6 address back and fail every send with `EINVAL`, which `localhost` reproduces on most modern systems
     * The queue of sends waiting on a cold-start lookup is bounded at 1000, dropping the oldest past that
* [@bdeitte](https://github.com/bdeitte) Make `close()` complete instead of hanging. The final buffered flush is now bounded at 5 seconds on every transport, and each of these previously hung a close: a stalled `cacheDns` lookup, a `tcp` connection that never completes, a socket the application had already destroyed, and a `uds` retry waiting out its backoff. Every queued send's callback fires exactly once, and sends issued after close fail fast rather than queueing
* [@bdeitte](https://github.com/bdeitte) Bound the memory a stalled or unreachable receiver can retain. `tcp` and `stream` sends are refused once more than `maxPendingWriteBytes` (new option, default 1 MiB) has been waiting to flush across an event-loop turn without draining, since Node otherwise queues writes without limit while a socket is connecting or its peer has stalled. Requiring a turn to pass is what keeps the cap off a healthy agent: a socket's pending bytes only fall once control returns to the event loop, so a synchronous loop of metrics accumulates its whole burst before any of it is flushed no matter how fast the agent reads. The `uds` sends waiting out a retry backoff are capped at 1000, dropping the oldest past that; `udsRetryOptions.retries` caps the attempts for one send, not how many sends retry at once
* [@bdeitte](https://github.com/bdeitte) Give every refused send a `HOTSHOTS_*` error code and count it as `packets_dropped_queue` rather than a writer error, and add the matching `bytes_dropped_queue` to the Datadog telemetry. A `cacheDns` cooldown refusal is the exception: it keeps the resolver's code on `code` and carries `HOTSHOTS_DNS_COOLDOWN` on `hotShotsCode`, which is what the telemetry split reads. NETWORKING.md lists each code and its telemetry bucket
* [@bdeitte](https://github.com/bdeitte) Deliver send failures on a later tick rather than on the same stack frame as the send. An `errorHandler` that emits a metric on failure can therefore no longer grow the stack without bound against a persistently failing transport
* [@bdeitte](https://github.com/bdeitte) Contain a user callback or `errorHandler` that throws, on every path that reaches one, reporting it with `console.error` and continuing. Most of those paths run inside an event emit, a timer, or a transport's write callback, so a throwing handler previously ended the process. It could also abandon the rest of a batch of sends, or leave `close()` with an open socket and a callback that never fires
