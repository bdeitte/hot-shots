...
* [@bdeitte](https://github.com/bdeitte) Bound and contain failures on the send path:
     ...
     * Contain a user callback or `errorHandler` that throws. Every path that calls back a batch of sends now reports the throw with `console.error` and continues. One bad callback can therefore no longer abandon the rest of the batch, or leave `close()` with an open socket and a callback that never fires. A throwing `errorHandler` is now contained on every path that reaches it, including the socket `error` event, the single-send failure, a failed socket creation or replacement, and a send refused after `close()`. Those calls run inside an event emit, a timer, or the transport's write callback, so the throw previously escaped as an uncaught exception and ended the process
