...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  // Preserve an explicit `sampleRate: 0` (validated above) — `||` would coerce 0 to 1
  // and silently emit every metric, defeating the user's intent to drop everything.
  this.sampleRate = (options.sampleRate === undefined || options.sampleRate === null) ?
    1 :
    options.sampleRate;
...
Client.prototype.sendStat = function (stat, value, type, sampleRate, tags, timestamp, callback) {
  ...
  // Resolve effective sampleRate. Use explicit undefined/null check (not ||) so that
  // a per-call sampleRate of 0 — or a client default of 0 — is preserved. `0` means
  // "always sample out", and the truthiness check below also uses an explicit
  // comparison so the 0 case takes the sampling branch.
  if (sampleRate === undefined || sampleRate === null) {
    sampleRate = this.sampleRate;
  }
  if (sampleRate < 1) {
    if (Math.random() < sampleRate) {
      // Note: when sampleRate is 0, Math.random() < 0 is always false, so we always
      // fall through to the sampled-out branch — which is the intended behavior.
      message += `|@${sampleRate}`;
  ...
  this.flushQueue((err) => {
    ...
    const waitForDrain = () => {
      ...
      if (this.messagesInFlight === 0) {
        ...
        // a real setTimeout — not setImmediate — covers the timer phase too. If a queued
        // send fires during this window, messagesInFlight will be > 0 and we loop.
        setTimeout(() => {
          if (this.messagesInFlight === 0) {
            finish();
          } else {
...
Client.prototype._close = function (callback) {
  ...
  if (this.errorHandler && this.socket) {
  ...
  try {
    ...
    // Remove the listeners we just attached so caller-owned streams don't accumulate
    // them across repeated client lifecycles or close retries.
    this.socket.removeListener('close', onClose);
    this.socket.removeListener('error', handleErr);
    handleErr(err);
  }
