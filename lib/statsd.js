...
/**
 * Stringify a value for use in user-facing warning messages. Falls back to String()
 * (and then a sentinel) if JSON.stringify throws — protects against BigInt, circular
 * refs, and other non-serializable values reaching the warning path.
 * @param {*} value
 * @returns {string}
 */
...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  // Warn loudly on clearly-invalid options (only validate values the user explicitly
  // passed). We log via console.error rather than throw to preserve backwards
  // compatibility — existing apps that happen to pass odd values keep working.
  // Invalid values are then cleared to undefined so the existing `|| <default>`
  // chains downstream pick up the safe fallback (e.g. `bufferFlushInterval: 'soon'`
  // would otherwise produce a hot setInterval loop).
  if (options.port !== undefined && options.port !== null) {
    const p = options.port;
    if (typeof p !== 'number' || !Number.isInteger(p) || p < 1 || p > 65535) {
      console.error(`hot-shots: 'port' should be an integer in [1, 65535], got ${safeStringify(p)} — using default`);
  ...
  if (options.sampleRate !== undefined && options.sampleRate !== null) {
    ...
    if (typeof s !== 'number' || Number.isNaN(s) || s <= 0 || s > 1) {
      console.error(`hot-shots: 'sampleRate' should be a number in (0, 1], got ${safeStringify(s)} — using default`);
  ...
  if (options.bufferFlushInterval !== undefined && options.bufferFlushInterval !== null) {
    ...
    if (typeof b !== 'number' || !Number.isFinite(b) || b <= 0 || b > 2147483647) {
      console.error('hot-shots: \'bufferFlushInterval\' should be a finite positive number ' +
        `<= 2147483647, got ${safeStringify(b)} — using default`);
  ...
  // True iff this.errorHandler is the same function currently attached as the
  // shared socket's 'error' listener. When true, _close's async socket-error path
  // must NOT call this.errorHandler again — the socket emit already delivered to
  // it.
  this._errorHandlerIsOnSocket = options._errorHandlerIsOnSocket !== undefined ?
    options._errorHandlerIsOnSocket :
...
Client.prototype.sendStat = function (stat, value, type, sampleRate, tags, timestamp, callback) {
  // Warn (don't throw) on per-call sampleRate of 0. To drop a metric, simply do not
  // call the method. Falling through preserves the historical behavior where 0 was
  // coerced to the client default via `||`.
  if (sampleRate === 0) {
    console.error('hot-shots: per-call \'sampleRate\' of 0 is meaningless; ' +
      'to drop a metric just don\'t call the method (treating as client default)');
  }
...
Client.prototype.send = function (message, tags, callback) {
  ...
  if (mergedTags.length > 0) {
    if (this.telegraf) {
      // Find the first colon — the boundary between metric name and value(s).
      // If the message has no colon (only possible from non-standard external
      // callers; every internal construction includes one), treat the whole
      // message as the "name" so the trailing colon matches the prior split-based
      // behavior `${parts[0]},${tagStr}:` exactly.
      let firstColon = message.indexOf(':');
      if (firstColon === -1) {
        firstColon = message.length;
      }
      ...
      // For messages with no colon, this appends ",tagStr:" — matching pre-fix output.
      message = firstColon === message.length ?
        `${message},${tagStr}:` :
        `${message.substring(0, firstColon)},${tagStr}${message.substring(firstColon)}`;
    } else {
...
Client.prototype.close = function (callback) {
  ...
  this.flushQueue((err) => {
    ...
    // and a fresh _drainPromise is allocated. Without the re-check loop we'd snapshot
...
Client.prototype._close = function (callback) {
  ...
  // local flag — NOT the persistent this._errorHandlerIsOnSocket — because non-child
  // clients remove their errorHandler from the socket below, which makes the
  // persistent flag stale during close.
  let errorHandlerOnSocketDuringClose = this._errorHandlerIsOnSocket;
...
const ChildClient = function (parent, options) {
  ...
  Client.call(this, {
    ...
    _errorHandlerIsOnSocket: options.errorHandler ?
      false :
      Boolean(parent._errorHandlerIsOnSocket),
    host        : parent.host,
    port        : parent.port,
