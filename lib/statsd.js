...
Client.prototype.flush = function (callback) {
  ...
  this.flushQueue(err => {
    ...
    const drainClients = new Set([this]);
    if (this.aggregator) {
      this.aggregator.activeClients.forEach(client => drainClients.add(client));
    }
    const pending = [];
...
Client.prototype.close = function (callback) {
  ...
  this.flushQueue((err) => {
    ...
    if (this.aggregator) {
      this.aggregator.activeClients.forEach(addDrainClient);
    }
