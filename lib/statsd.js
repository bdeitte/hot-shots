...
// DogStatsD metric type codes as they appear on the wire.
...
Client.prototype.sendStat = function (stat, value, type, sampleRate, tags, timestamp, cardinality, callback) {
  ...
  if (this.aggregator && !this.aggregator.closed &&
      (type === METRIC_TYPE_COUNT || type === METRIC_TYPE_GAUGE || type === METRIC_TYPE_SET) &&
      timestamp === undefined &&
      (!callSampleRate || callSampleRate >= 1) &&
      (type === METRIC_TYPE_SET || (typeof value === 'number' && !Number.isNaN(value)))) {
  ...
  if (type === METRIC_TYPE_GAUGE && this.aggregator && !this.aggregator.closed) {
    this.aggregator.flushContext(this, this.prefix + sanitizedStat + this.suffix, METRIC_TYPE_GAUGE, tags, cardinality);
  }
