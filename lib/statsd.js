...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  if (options.isChild) {
    ...
    if (options.datadog === true && this.telegraf) {
      ...
      this.datadog = helpers.detectDatadogMode(this.telegraf);
    }
  ...
  // Telemetry options (Datadog-specific). Defaults on whenever datadog mode is
  // active — which only happens via an explicit `datadog: true` or a DD_* env var,
  // never from the `uds` protocol alone. Explicit includeDatadogTelemetry always
  // wins. Always disabled for telegraf/mock/child clients.
  const telemetryDefault = this.datadog === true;
