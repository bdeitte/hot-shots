...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  this.errorListener = this.errorHandler ?
    (err) => callErrorHandler(this, err, 'the socket error event') :
    undefined;
...
Client.prototype._send = function (message, callback) {
  ...
  if (this.dnsError) {
    ...
    if (callback) {
      ...
      return callErrorHandler(this, this.dnsError, 'a cached DNS error');
    }
    throw this.dnsError;
  }
...
Client.prototype.sendMessage = function (message, callback, isTelemetry) {
  ...
  if (socketWasMissing) {
    ...
    if (callback) {
      ...
      return callErrorHandler(this, error, 'a failed socket creation');
    } else {
  ...
  // drains it, tripping the force-close warning on every close. A warm address,
  // an IP literal, and non-cacheDns transports are untouched.
  if (this.socket && typeof this.socket.isDnsSendBlocked === 'function' && this.socket.isDnsSendBlocked()) {
    ...
    if (callback) {
      ...
      setImmediate(() => callErrorHandler(this, error, 'a send refused after close'));
    } else {
    ...
    if (callback) {
      ...
      if (this.errorHandler) {
        ...
        try {
          ...
          console.error('hot-shots: errorHandler threw inside a send failure; ' +
            `original error: ${errFormatted && errFormatted.message}; ` +
            `handler error: ${handlerErr && handlerErr.message}`);
        }
      } else {
    ...
    if (err && constants.CLOSE_CONTINUE_CODES.includes(refusalCode(err))) {
      if (this.errorHandler) {
        try {
          ...
          console.error('hot-shots: errorHandler threw inside a refused final flush; ' +
            `original error: ${err && err.message}; handler error: ${handlerErr && handlerErr.message}`);
        }
      } else {
...
Client.prototype._close = function (callback) {
  ...
  try {
    ...
    if (this.errorHandler && !this.isChild) {
      safeOn('error', this.errorListener);
    }
    handleErr(err);
  }
...
function protocolErrorHandler(client, protocol, err) {
  ...
  if (client.errorHandler && client.socket) {
    client.socket.removeListener('error', client.errorListener);
  }
  ...
  if (client.errorHandler) {
    client.socket.on('error', client.errorListener);
  } else {
