...
Client.prototype.close = function (callback) {
  ...
  // cold-start DNS lookup that never resolves would hang close() here, before
  // the drain logic below is even installed. Budgeted separately from (and much
  // larger than) the drain wait; see DNS_CLOSE_FLUSH_TIMEOUT in lib/constants.js.
  ...
  this.flushQueue((err) => {
    ...
    // A flush the DNS layer refused outright must not abort the close: the
    // socket still needs closing (and on a repeat close, finish() still needs to
    // clear a stale messagesInFlight/drainPromise) and the caller's callback
    // still needs to fire. Report and carry on; any other flush error keeps the
    // existing early-return. All three refusal codes land here — cancelled
    // mid-lookup by the guard above, rejected by a prior close(), or dropped
    // because a failed lookup is still in its cooldown — and mean the same thing
    // to close(): nothing was handed to the socket, so there is nothing to wait
    // on and no reason to leave the socket open.
    if (err && constants.DNS_REFUSED_CODES.includes(err.code)) {
      if (this.errorHandler) {
        try {
          this.errorHandler(err);
        } catch (handlerErr) {
