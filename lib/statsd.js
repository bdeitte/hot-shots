...
Client.prototype.sendMessage = function (message, callback, isTelemetry) {
  ...
  const handleCallback = (err, bytes) => {
    ...
    if (errFormatted) {
      ...
      // Carried through the wrapping so callers see the same refusal marker the
      // transport set. Only the cacheDns cooldown sets it today, where `code`
      // stays the resolver's own.
      if (err.hotShotsCode) {
        errFormatted.hotShotsCode = err.hotShotsCode;
      }
