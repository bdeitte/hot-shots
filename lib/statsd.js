...
Client.prototype.sendMessage = function (message, callback, isTelemetry) {
  ...
  // reads it, and a resending errorHandler would refill it as fast as close()
  ...
  const handleCallback = (err, bytes) => {
    // Clamp at zero: close()'s finish() force-zeroes messagesInFlight once it gives
    // up waiting on drain, but a queue-overflow batch's deferred (setImmediate)
    // callbacks can still be scheduled and fire afterwards. Without the clamp those
    // late decrements would drive the counter negative, and the `=== 0` check just
    ...
    if (errFormatted) {
      ...
      // cause: a client-side capacity or lifecycle refusal (the cacheDns queue
      // overflowing, or a closed client) counts as a queue drop, while anything
...
Client.prototype.close = function (callback) {
  ...
  // logic below is even installed. Every transport has a way to get stuck: a
  // cold-start DNS lookup that never resolves, or a tcp/stream write sitting in
  // a socket whose connect never completes. The guard below bounds all of them.
  let flushSettled = false;

  /**
   * Runs the rest of close() once the final flush has settled, whether that was
   * the flush calling back or the guard below giving up on it. Guarded so it
   * runs at most once.
  ...
  const onFlushSettled = (err) => {
    ...
    // must not abort the close: the socket still needs closing (and on a repeat
    // close, finish() still needs to clear a stale messagesInFlight/
    // drainPromise) and the caller's callback still needs to fire. Report and
    // carry on; any other flush error keeps the existing early-return.
  ...
  // timer may be the only thing left holding the process open, and close() must
...
Client.prototype._close = function (callback) {
  ...
  // real 'close' from the underlying socket, and a caller-supplied stream may
  ...
  // first call — so onClose would wait on an event that never arrives. Note it
...
function cancelDnsPendingSends(client) {
  if (client.socket && typeof client.socket.cancelPendingSends === 'function') {
    ...
    // covers anything else the cancel path could throw, because both callers run
    // inside close() and an escaping throw there would skip the socket close and
    // the caller's callback, hanging close() for good.
    try {
