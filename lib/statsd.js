...
Client.prototype.sendMessage = function (message, callback, isTelemetry) {
  ...
  // Client.close() latches the transport's cacheDns queue closed once it gives
  // up waiting on a stuck lookup (see cancelDnsPendingSends). Check that latch
  // BEFORE incrementing messagesInFlight below: incrementing it here and
  // relying on the transport to reject the send would leave messagesInFlight
  // transiently non-zero the instant close()'s finish() checks it — a
  // resending errorHandler (the documented "emit a metric on send failure"
  // pattern) would otherwise refill messagesInFlight just as fast as close()
  // drains it, spuriously tripping the force-close warning on every close.
  // Skipping the increment entirely here keeps the counter accurate no matter
  // how many times a rejected send's callback/errorHandler re-sends. Only
  // affects the cacheDns cold-start queueing path — a warm resolved address,
  // an IP literal, or a non-cacheDns transport are untouched, matching what
  // socket.close() already leaves alone.
  if (this.socket && typeof this.socket.isDnsSendBlocked === 'function' && this.socket.isDnsSendBlocked()) {
    ...
    // This send arrived AFTER close() already latched the DNS queue closed -
    // not one still queued behind a lookup that close() cancelled mid-flight
    // (that case keeps DNS_CANCELLED_CODE, which close()'s flush-error branch
    // below matches on to let a cancelled final flush through without
    // aborting the socket close).
    error.code = constants.DNS_CLOSED_CODE;
    debug('hot-shots sendMessage: rejecting send, this client\'s DNS-cached queue was closed');
    if (this.telemetry && !isTelemetry) {
      this.telemetry.recordBytesDroppedWriter(messageBytes);
    }
    // Deferred, not synchronous, for the same reason the transport's own
    // post-cancellation rejection is deferred (lib/transport.js): a
    // callback/errorHandler invoked here can call send() again, and calling
    // back synchronously would re-enter this same branch on the same stack
    // frame with no bound - the Task 3 recursion hazard class. Deferring
    // breaks that into separate ticks, matching the queue-overflow drop path.
    if (callback) {
      ...
      // Emit error ourselves on the socket for backwards compatibility, matching
      // handleCallback's no-callback/no-errorHandler fallback below - otherwise a
      // `client.socket.on('error', ...)` listener never sees a post-close send
      // failure on this path. Deferred for the same reentrancy reason as the
      // callback/errorHandler branches above: a socket 'error' listener can call
      // send() again synchronously.
      if (this.socket) {
  ...
  const handleCallback = (err, bytes) => {
    ...
    if (errFormatted) {
      ...
      // Track bytes dropped (only for non-telemetry messages). A queue-capacity
      // drop (e.g. the cacheDns pending queue overflowing) is backpressure, not
      // a writer/transport failure, so it's counted separately.
      if (this.telemetry && !isTelemetry) {
        if (err.code === constants.DNS_QUEUE_FULL_CODE) {
...
Client.prototype.close = function (callback) {
  ...
  // drain logic below is even installed. This is a separate, larger budget than
  // the drain wait below (closingFlushInterval * 11, ~550ms by default): a
  // first-ever lookup taking a few hundred ms is ordinary and must not lose the
  // final flush, so the guard needs enough room to let that resolve. It exists
  // only to bound the pathological hung-resolver case, where waiting several
  // seconds before giving up on the flush is an acceptable tradeoff. See
  // DNS_CLOSE_FLUSH_TIMEOUT in lib/constants.js.
  const flushGuard = setTimeout(() => {
    cancelDnsPendingSends(this);
  }, constants.DNS_CLOSE_FLUSH_TIMEOUT);

  // flush the queue one last time, if needed
  this.flushQueue((err) => {
    clearTimeout(flushGuard);

    // A flush rejected by the DNS queue must not abort the close: the socket
    // still needs closing (or, on a repeat close, finish() still needs to run
    // so a stale messagesInFlight/drainPromise from an earlier close gets
    // cleared) and the caller's callback still needs to fire. Report the
    // dropped metrics and carry on. Any other flush error keeps the existing
    // early-return behavior. Both DNS codes land here: DNS_CANCELLED_CODE is
    // the guard above cancelling an in-flight lookup, DNS_CLOSED_CODE is this
    // flush arriving after a PRIOR close() already latched the queue shut
    // (e.g. a second close() on the same client) - for close()'s purposes
    // they mean the same thing, so both must pass through identically.
    if (err && (err.code === constants.DNS_CANCELLED_CODE || err.code === constants.DNS_CLOSED_CODE)) {
      if (this.errorHandler) {
        try {
          this.errorHandler(err);
        } catch (handlerErr) {
