...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  debug('Client initialized: protocol=%s, host=%s, port=%s, prefix=%s, maxBufferSize=%s, mock=%s',
...
Client.prototype.sendStat = function (stat, value, type, sampleRate, tags, callback) {
  ...
  if (sampleRate && sampleRate < 1) {
    if (Math.random() < sampleRate) {
      message += `|@${sampleRate}`;
      debug('sendStat: sampled in - stat=%s, type=%s, sampleRate=%s', stat, type, sampleRate);
    } else {
      // don't want to send if we don't meet the sample ratio
      debug('sendStat: sampled out - stat=%s, type=%s, sampleRate=%s', stat, type, sampleRate);
      return callback ? callback() : undefined;
    }
  }
  debug('sendStat: sending message=%s, tags=%j', message, tags);
...
Client.prototype._send = function (message, callback) {
  ...
  // throw it on
  if (this.dnsError) {
    debug('_send: DNS error - %s', this.dnsError);
    if (callback) {
  ...
  if (!this.mock) {
    if (this.maxBufferSize === 0) {
      debug('_send: sending immediately (no buffering) - message=%s', message);
      this.sendMessage(message, callback);
    } else {
      debug('_send: enqueueing for buffer - message=%s', message);
      this.enqueue(message, callback);
    }
  } else {
    debug('_send: mock mode - buffering message=%s', message);
...
Client.prototype.enqueue = function (message, callback) {
  ...
  if (this.bufferHolder.buffer.length + messageToAdd.length > this.maxBufferSize) {
           debug('enqueue: buffer full (%d + %d > %d), flushing',
    ...
    debug('enqueue: added to buffer, new size=%d', this.bufferHolder.buffer.length);
    if (callback) {
      callback();
    }
  }
...
Client.prototype.flushQueue = function (callback) {
  if (this.bufferHolder.buffer && this.bufferHolder.buffer.length > 0) {
    debug('flushQueue: flushing %d bytes', this.bufferHolder.buffer.length);
  }
...
Client.prototype.sendMessage = function (message, callback) {
  ...
  if (socketWasMissing && (this.protocol === PROTOCOL.TCP || this.protocol === PROTOCOL.UDS)) {
    debug('sendMessage: socket missing, attempting to recreate for protocol=%s', this.protocol);
    trySetNewSocket(this);
    if (this.socket) {
      // On success, add custom TCP/UDS error handling.
      maybeAddProtocolErrorHandler(this, this.protocol, Date.now());
      debug('sendMessage: socket recreated successfully');
    }
  }

  if (socketWasMissing) {
    const error = new Error('Socket not created properly. Check previous errors for details.');
    debug('sendMessage: socket creation failed - %s', error.message);
    if (callback) {
  ...
  const handleCallback = (err, bytes) => {
    ...
    if (errFormatted) {
      ...
      debug('sendMessage: error sending - %s (code: %s)', err.message, err.code);
      ...
      debug('sendMessage: successfully sent %d bytes', message.length);
    }
    if (callback) {
  ...
  try {
    this.messagesInFlight++;
    debug('sendMessage: sending %d bytes via %s transport (messagesInFlight=%d)',
      message.length, this.protocol, this.messagesInFlight);
    this.socket.send(Buffer.from(message), handleCallback);
  } catch (err) {
    debug('sendMessage: exception during send - %s', err.message);
    handleCallback(err);
  }
};
