...
Client.prototype.close = function (callback) {
  ...
  this.flushQueue((err) => {
    ...
    // still needs closing (and on a repeat close, finish() still needs to clear
    // a stale messagesInFlight/drainPromise) and the caller's callback still
    // needs to fire. Report and carry on; any other flush error keeps the
    // existing early-return. Both DNS codes land here — cancelled mid-lookup by
    // the guard above, or rejected by a prior close() — and mean the same thing
    // to close().
