...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  if (options.sampleRate !== undefined && options.sampleRate !== null) {
    ...
    // Reject sampleRate <= 0: dropping every metric is not a valid use case (just don't
    // call the metric methods). Accept (0, 1] only.
...
Client.prototype.sendStat = function (stat, value, type, sampleRate, tags, timestamp, callback) {
  // Reject per-call sampleRate of 0 for the same reason the constructor does:
  // dropping every metric is not a valid use case (just don't call the method).
  // Validate BEFORE telemetry recording so a rejected call does not count in the
  // datadog.dogstatsd.client.metrics* telemetry.
  if (sampleRate === 0) {
    throw new TypeError('hot-shots: per-call \'sampleRate\' must not be 0; ' +
      'to drop a metric just don\'t call the method');
  }
...
Client.prototype._close = function (callback) {
  ...
  // Children share the parent's socket and inherit the parent's `errorHandler`. Adding
  // our own 'error' listener (handleErr) here would double-deliver close-time errors:
  // once via the parent's still-attached errorHandler, and once via handleErr routing
  // to this.errorHandler (which IS the parent's). Match the prior child close behavior
  // (synchronously close the socket and invoke the callback) without touching the
  // shared listener set.
  if (this.isChild) {
    let closeErr = null;
    try {
    ...
    if (closeErr) {
      const errMessage = `Error closing hot-shots socket: ${closeErr}`;
      if (callback) {
  ...
  if (this.errorHandler && this.socket && !this.isChild) {
    safeRemoveListener('error', this.errorHandler);
  }

  // handle error and close events. Always attach a self-removing 'close' listener —
  // even when no caller callback was supplied — so we can remove handleErr after close
  // completes. Without this cleanup, caller-owned streams (which outlive our client)
  // would accumulate handleErr listeners every time a client is created and closed
  // against the same stream. (The transport interface does not expose `once`, so we
  // emulate one-shot semantics by removing the listener inside its own handler.)
  safeOn('error', handleErr);
  const onClose = (err) => {
    safeRemoveListener('close', onClose);
    safeRemoveListener('error', handleErr);
