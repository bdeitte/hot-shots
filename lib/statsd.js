...
Client.prototype.enqueue = function (message, callback) {
  const separator = this.bufferHolder.buffer === '' ? '' : '\n';
  const messageToAdd = separator + message;

  // Check if adding this message would exceed the limit
  // If so, flush the existing buffer first before adding the new message
  if (this.bufferHolder.buffer.length > 0 &&
      this.bufferHolder.buffer.length + messageToAdd.length > this.maxBufferSize) {
    this.flushQueue();
  }

  // Add the message to the (possibly just-flushed) buffer
  this.bufferHolder.buffer += this.bufferHolder.buffer === '' ? message : '\n' + message;
