...
Client.prototype.sendMessage = function (message, callback, isTelemetry) {
  ...
  const handleCallback = (err, bytes) => {
    ...
    if (errFormatted) {
      ...
      if (this.telemetry && !isTelemetry) {
        if (constants.REFUSED_CODES.includes(err.code)) {
          this.telemetry.recordBytesDroppedQueue(messageBytes);
        } else {
          this.telemetry.recordBytesDroppedWriter(messageBytes);
        }
...
Client.prototype.close = function (callback) {
  ...
  const onFlushSettled = (err) => {
    ...
    if (err && constants.CLOSE_CONTINUE_CODES.includes(err.code)) {
      if (this.errorHandler) {
        try {
          this.errorHandler(err);
        } catch (handlerErr) {
...
/**
 * Calls a client's errorHandler, containing a throw so it cannot escape. Every
 * caller reaches this from an event emit, a timer, or a transport's I/O
 * callback, where nothing above would catch it and the process would end.
 * Callers must check that client.errorHandler is set.
 * @param client The client whose errorHandler should run.
 * @param err {Error} The error to report to it.
 * @param context {String} A label for the call site, used in the log message.
 * @returns {void}
 */
function callErrorHandler(client, err, context) {
  try {
