...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  // it. ChildClient propagates this from the parent: an inheriting child copies
  // the parent's value (same fn → same on-socket status); an overriding child
  // sets false (the override is never registered on the socket).
