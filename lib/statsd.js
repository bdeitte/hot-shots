...
Client.prototype.sendMessage = function (message, callback, isTelemetry) {
  ...
  const handleCallback = (err, bytes) => {
    ...
    if (errFormatted) {
      ...
      if (self.telemetry && !isTelemetry) {
        self.telemetry.recordBytesDroppedWriter(messageBytes);
      }
      // handle TCP/UDS error that requires socket replacement when we are not
      // emitting the `error` event on `this.socket`
      if ((self.protocol === PROTOCOL.TCP || self.protocol === PROTOCOL.UDS) && (callback || self.errorHandler)) {
        protocolErrorHandler(self, self.protocol, err);
      }
    } else {
      ...
      if (self.telemetry && !isTelemetry) {
        self.telemetry.recordBytesSent(messageBytes);
      }
    }
    if (callback) {
