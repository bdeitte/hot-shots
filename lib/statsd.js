...
Client.prototype.sendStat = function (stat, value, type, sampleRate, tags, timestamp, cardinality, callback) {
  ...
  if (this.aggregator && !this.aggregator.closed &&
      (type === 'c' || type === 'g' || type === 's') &&
      timestamp === undefined &&
      (!callSampleRate || callSampleRate >= 1) &&
      (type === 's' || (typeof value === 'number' && !Number.isNaN(value)))) {
...
Client.prototype.flush = function (callback) {
  ...
  if (this.aggregator) {
    // Guard like close() and the flush interval do: a synchronous send throw must
    // not escape flush() (orphaning the callback) after the aggregator has already
    // swapped out its contexts map and dropped the remaining aggregated metrics.
    try {
      this.aggregator.flush(involvedClients);
    } catch (err) {
      if (this.errorHandler) {
        try {
          this.errorHandler(err);
          ...
          // Preserve the original flush error so the root cause is not masked by a buggy handler.
          console.error('hot-shots: errorHandler threw inside flush aggregation flush; ' +
            `original error: ${err && err.message}; handler error: ${handlerErr && handlerErr.message}`);
        }
      } else {
        console.error(`hot-shots: flush aggregation flush threw: ${err && err.message}`);
      }
    }
  }
  if (!callback) {
  ...
  this.flushQueue(err => {
    ...
    // The aggregator may have routed metrics through this and/or child clients. In
    // unbuffered mode (maxBufferSize === 0) those sends are asynchronous, as is any
    // earlier unbuffered send, so wait for every involved client (plus this one) to
    // drain before signaling completion. This keeps the documented serverless
    // "flush then exit" use case correct rather than firing the callback early.
    involvedClients.add(this);
    const pending = [];
    involvedClients.forEach(client => {
...
Client.prototype.close = function (callback) {
  ...
  // child), so collect those clients and drain them below before closing the
  // shared socket — otherwise child-recorded metrics could be dropped.
  const aggregatorClients = new Set();
  if (this.aggregator) {
    try {
      this.aggregator.flush(aggregatorClients);
    } catch (err) {
      if (this.errorHandler) {
        try {
          this.errorHandler(err);
  ...
  this.flushQueue((err) => {
    ...
    // Drain this client plus any other clients the aggregator flush sent through
    // (child clients track their own messagesInFlight). Closing the shared socket
    // before these drain would drop their in-flight aggregated metrics.
    const drainClients = [this];
    const addDrainClient = client => {
      if (client !== this && drainClients.indexOf(client) === -1) {
        drainClients.push(client);
      }
    };
    aggregatorClients.forEach(addDrainClient);
    // Also drain any client an interval-driven flush routed through before close()
    // ran — those sends pass no involvedClients set, so without this they would
    // escape drain coordination and the shared socket could be destroyed under them.
    if (this.aggregator) {
      this.aggregator.routedClients.forEach(addDrainClient);
    }
...
function setupDatadogGlobalTags(client, options) {
  ...
  // Child clients already inherit DD_TAGS and the DD_* mapping through the parent's
  // resolved globalTags; re-applying here would put the env tags on the winning
  // side of the child's own globalTags merge and clobber a child's explicit
  // override of any matching key.
  if (options.isChild) {
    return;
  }
...
function setupAggregation(client, options) {
  ...
  // Client-side aggregation is a DogStatsD feature; disable it for telegraf,
  // matching the DogStatsD-only pattern used by event/check/telemetry rather than
  // silently delaying and merging a telegraf client's metrics.
  if (client.telegraf) {
    console.error('hot-shots: client-side aggregation is a DogStatsD feature and is not ' +
      'supported with telegraf; aggregation disabled');
    client.aggregator = null;
    return;
  }
  const aggregationOptions = typeof options.aggregation === 'object' ? options.aggregation : {};
  let flushInterval = aggregationOptions.flushInterval;
  // Validate flushInterval like bufferFlushInterval: reject non-finite, <= 0, or
  // values above setTimeout's signed-32-bit max (Node clamps oversized delays to
  // 1ms, creating a CPU-pegging hot flush loop). Invalid values fall back to default.
  if (flushInterval !== undefined && flushInterval !== null) {
    if (typeof flushInterval !== 'number' || !Number.isFinite(flushInterval) ||
        flushInterval <= 0 || flushInterval > 2147483647) {
      console.error('hot-shots: aggregation flushInterval should be a finite positive number ' +
        `<= 2147483647, got ${flushInterval} — using default`);
      flushInterval = undefined;
    }
  }
  client.aggregator = new Aggregator({ flushInterval: flushInterval });
