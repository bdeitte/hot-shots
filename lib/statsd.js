...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  // UDS socket options bag (non-backwards compatible): includes path and retry settings
  this.udsSocketOptions = options.udsSocketOptions || (this.path ? { path: this.path } : undefined);
  ...
  // Remove legacy top-level UDS retry fields in favor of udsSocketOptions
...
function protocolErrorHandler(client, protocol, err) {
  ...
  const newSocket = createTransport(client, {
    host: client.host,
    path: client.path,
    udsSocketOptions: client.udsSocketOptions,
    port: client.port,
