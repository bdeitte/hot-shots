...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  // Telemetry options (Datadog-specific). Defaults on when datadog mode is active
  // AND there's a strong Datadog signal — i.e. an explicit `datadog: true` or a
  // DD_* env var. It deliberately does NOT default on for datadog mode that was
  // auto-detected solely from the `uds` protocol, so existing uds users don't
  // silently start emitting telemetry on upgrade. Explicit includeDatadogTelemetry
  // always wins. Always disabled for telegraf/mock/child clients.
