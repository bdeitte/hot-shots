...
class Telemetry {
   ...
   * @param {Function} [options.errorHandler] - Optional callback for telemetry flush errors
   */
  constructor(options) {
      ...
      try {
        ...
        if (this.errorHandler) {
          try {
            ...
            console.error('hot-shots telemetry: errorHandler threw inside flush interval; ' +
              `original error: ${err && err.message}; handler error: ${handlerErr && handlerErr.message}`);
          }
        } else {
          console.error(`hot-shots telemetry: flush interval threw: ${err && err.message}`);
        }
      }
    }, this.flushInterval);
  ...
  /**
   * Record bytes dropped because a client-side queue was full (backpressure), as
   * opposed to a writer/transport error. Increments the overall
   * bytesDropped/packetsDropped totals plus the queue-specific pair. This
   * mirrors how recordBytesDroppedWriter splits out the writer pair.
   * @param {number} bytes - Number of bytes dropped
   */
  recordBytesDroppedQueue(bytes) {
    this.bytesDropped += bytes;
    this.bytesDroppedQueue += bytes;
    this.packetsDropped++;
    this.packetsDroppedQueue++;
    debug('hot-shots telemetry: recordBytesDroppedQueue bytes=%d, totalDropped=%d', bytes, this.bytesDropped);
  }
  ...
  flush(callback) {
    ...
    // Bytes dropped by queue
    if (this.bytesDroppedQueue > 0) {
      messages.push(this.formatMetric('bytes_dropped_queue', this.bytesDroppedQueue));
    }
