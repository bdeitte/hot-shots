 ...
 *
 * Every fan-out in this file calls a batch of send callbacks in a loop, and each
 * one lands in user code - a per-send callback, or the client's errorHandler. An
 * escaping throw would abandon the rest of the batch, and those entries have
 * already been spliced out of the queue that held them, so nothing would ever
 * call them back. That breaks the exactly-once contract the client's drain logic
 * depends on, and from close()'s cancel path it hangs close() itself before it
 * can shut the socket.
 *
...
// apiece; invokeCallback keeps one client's throwing callback from abandoning
 ...
 * until the process throws RangeError or stops responding.
 *
...
const createTcpTransport = args => {
  // With no host, net.connect would target `localhost`, costing a DNS lookup
  // and resolving to ::1 before 127.0.0.1 on most systems - unreachable when
  // the agent listens on IPv4 only. Default to the IPv4 loopback literal
  ...
  // (see #222). Node 20+ does this by default, but set it explicitly so the
  // behavior holds even when the default is turned off, such as by running
...
const createUdpTransport = args => {
  ...
  const dnsResolutionData = {
    ...
    // resolver (cached NXDOMAIN, SERVFAIL) degrades to one lookup per send, the
    // per-packet behavior this work exists to eliminate. See cooldownMs for how
    // long, and why it ramps rather than sitting at a flat TTL.
  ...
  const reportRefreshFailure = (error) => {
    ...
    try {
      ...
      // lookup callback, and on the synchronous-throw path out of send() itself
      // - where the send has already been handed to the socket, so the client
      // would call back twice for the same message.
   ...
   *
   * A flat full-TTL cooldown would make the common cold start - a process that
   * comes up before its resolver does - lose every metric for a full TTL
   * (60s by default) to a blip that cleared in a second, since the cold path has
   * no cached address to fall back on. Ramping from a short first wait keeps the
   * anti-thrash property that matters (never one lookup per send) while letting
   * a brief outage recover in about a second.
  ...
  const startLookup = () => {
    ...
    try {
      // Constrained to the socket's family. Without it getaddrinfo is free to
      // answer a hostname with an address the socket cannot send to - `localhost`
      // resolves to ::1 on most modern systems, and handing that to the udp4
      // socket hostnames default to fails every send with EINVAL.
      ...
      if (isRefresh) {
        ...
        // throw again, and recurse without bound until the process stops responding.
  ...
  const sendUsingDnsCache = (callback, buf) => {
    ...
    if (overflow.length > 0) {
      ...
      // until a RangeError (stack overflow) or a process that stops responding.
      setImmediate(() => {
  ...
  return {
    ...
    cancelPendingSends: (error) => {
      ...
      // Belt-and-braces re-check, not a loop. The latch above is set before the
      // flush and sendUsingDnsCache checks it before every push, so nothing
      // should be able to land here; this only guards against a future edit
      // reordering that. Bounded by the same latch, so it cannot repeat.
      if (dnsResolutionData.pending.length > 0) {
        flushPending(error);
      }
    },
