...
const createStreamTransport = (args) => {
  ...
  // user-supplied stream with no listener crashes the process. We use a named
  // function (not anonymous) so close() can `removeListener` it — preventing
  // accumulation if multiple StatsD instances reuse the same stream over their
  // lifetimes.
