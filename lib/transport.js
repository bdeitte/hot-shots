...
const createUdsTransport = args => {
  ...
  const udsOpts = args.udsSocketOptions || {};
  const udsPath = (udsOpts.path || args.path) ? (udsOpts.path || args.path) : UDS_PATH_DEFAULT;
  ...
  const maxRetries = udsOpts.retries !== undefined ? udsOpts.retries : 3;
  const initialDelay = udsOpts.retryDelay !== undefined ? udsOpts.retryDelay : 100;
  const maxDelay = udsOpts.maxRetryDelay !== undefined ? udsOpts.maxRetryDelay : 1000;
  const backoffFactor = udsOpts.backoffFactor !== undefined ? udsOpts.backoffFactor : 2;

  const sendWithRetry = (buf, callback, attempt = 0) => {
    console.log(`MRJN Sending packet attempt: #${attempt}, buf: ${buf.toString().slice(0, 10)}`);
    socket.send(buf, (err) => {
      console.log(`MRJN Received callback for packet attempt: #${attempt}, err: ${err}, buf: ${buf.toString().slice(0, 10)}`);
      if (err && attempt < maxRetries) {
        const delay = Math.min(initialDelay * Math.pow(backoffFactor, attempt), maxDelay);
        console.warn(`MRJN Retrying send with retry #${attempt} in ${delay}ms`);
        setTimeout(() => {
