...
const createTcpTransport = args => {
  debug('createTcpTransport: connecting to %s:%s', args.host, args.port);
  ...
  return {
    ...
    send: (buf, callback) => {
      debug('TCP transport: sending %d bytes to %s:%s', buf.length, args.host, args.port);
      socket.write(addEol(buf), 'ascii', (err) => {
        if (err) {
          debug('TCP transport: send error - %s', err.message);
        } else {
          debug('TCP transport: send successful');
        }
        if (callback) {
          callback(err);
        }
    ...
    close: () => {
      debug('TCP transport: closing connection');
      socket.destroy();
    },
...
const createUdpTransport = args => {
  debug('createUdpTransport: creating socket for %s:%s (cacheDns=%s)', args.host, args.port, args.cacheDns);
  ...
  const sendUsingDnsCache = (callback, buf) => {
    ...
    if (dnsResolutionData.resolvedAddress === undefined || (now - dnsResolutionData.timestamp > args.cacheDnsTtl)) {
      debug('UDP transport: performing DNS lookup for %s', args.host);
      dns.lookup(args.host, (error, address) => {
        if (error) {
          debug('UDP transport: DNS lookup error - %s', error.message);
          callback(error);
          return;
        }
        debug('UDP transport: DNS resolved %s to %s', args.host, address);
        dnsResolutionData.resolvedAddress = address;
        dnsResolutionData.timestamp = now;
        try {
          debug('UDP transport: sending %d bytes to %s:%s', buf.length, dnsResolutionData.resolvedAddress, args.port);
          socket.send(buf, 0, buf.length, args.port, dnsResolutionData.resolvedAddress, (err) => {
            if (err) {
              debug('UDP transport: send error - %s', err.message);
            } else {
              debug('UDP transport: send successful (note: UDP does not guarantee delivery)');
            }
            if (callback) {
              callback(err);
            }
          ...
          debug('UDP transport: send exception - %s', socketError.message);
          callback(socketError);
        }
      });
    } else {
      debug('UDP transport: using cached DNS address %s', dnsResolutionData.resolvedAddress);
      try {
        debug('UDP transport: sending %d bytes to %s:%s', buf.length, dnsResolutionData.resolvedAddress, args.port);
        socket.send(buf, 0, buf.length, args.port, dnsResolutionData.resolvedAddress, (err) => {
          if (err) {
  ...
  return {
    ...
    send: function (buf, callback) {
      if (args.cacheDns) {
        ...
        try {
          debug('UDP transport: sending %d bytes to %s:%s (no DNS cache)', buf.length, args.host, args.port);
    ...
    close: () => {
      debug('UDP transport: closing socket');
      socket.close();
    },
    unref: socket.unref.bind(socket)
  };
...
const createUdsTransport = args => {
  ...
  debug('createUdsTransport: connecting to %s', udsPath);
  const socket = unixDgram.createSocket('unix_dgram');

  try {
    socket.connect(udsPath);
    debug('createUdsTransport: connected successfully');
  } catch (err) {
    debug('createUdsTransport: connection failed - %s', err.message);
    socket.close();
    throw err;
  }
  ...
  const sendWithRetry = (buf, callback, attempt = 0) => {
    if (attempt === 0) {
      debug('UDS transport: sending %d bytes', buf.length);
    } else {
      debug('UDS transport: retry attempt %d/%d', attempt, maxRetries);
    }
    socket.send(buf, (err) => {
      if (err && isRetryableUdsError(err) && attempt < maxRetries) {
        ...
        debug('UDS transport: retryable error (%s), retrying after %dms', err.message || err.code || err, delay);
        setTimeout(() => sendWithRetry(buf, callback, attempt + 1), delay);
      } else if (err) {
        debug('UDS transport: send error - %s (attempts: %d)', err.message || err.code || err, attempt + 1);
        if (typeof callback === 'function') {
          callback(err);
        }
      } else {
        debug('UDS transport: send successful (attempts: %d)', attempt + 1);
        if (typeof callback === 'function') {
          callback(err);
        }
      }
...
const createStreamTransport = (args) => {
  ...
  debug('createStreamTransport: using provided stream');
  ...
  return {
    ...
    send: (buf, callback) => {
      debug('Stream transport: sending %d bytes', buf.length);
      stream.write(addEol(buf), (err) => {
        if (err) {
          debug('Stream transport: send error - %s', err.message);
        } else {
          debug('Stream transport: send successful');
        }
        if (callback) {
          callback(err);
        }
    ...
    close: () => {
      debug('Stream transport: closing stream');
      stream.destroy();
