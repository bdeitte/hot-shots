...
const { DNS_CANCELLED_CODE, DNS_CLOSED_CODE, DNS_COOLDOWN_BASE_MS, DNS_COOLDOWN_CODE,
  DNS_COOLDOWN_MAX_MS, DNS_LOOKUP_FAILED_CODE, DNS_MAX_PENDING,
  DNS_QUEUE_FULL_CODE, MAX_PENDING_WRITE_BYTES, PROTOCOL, UDS_MAX_PENDING_RETRIES,
  UDS_RETRY_CANCELLED_CODE, UDS_RETRY_QUEUE_FULL_CODE, WRITE_QUEUE_FULL_CODE,
  dnsQueueClosedMessage } = require('./constants');
// helpers requires only constants and fs, so this does not create a cycle.
const { callErrorHandler } = require('./helpers');
 ...
 * stack frame as the send. Real I/O errors from dgram and net already satisfy
 * it by arriving from the event loop. unix-dgram does not: it calls back
 * inside send(), so the uds transport routes its failures through here too.
 *
 ...
 * already has more than its pending-byte cap sitting unflushed in its socket.
 * @param {string} label - transport name, for the message
 * @param {number} buffered - unflushed byte count at the time of the refusal
 * @param {number} limit - the cap that was exceeded
 * @returns {Function} a factory returning the coded backpressure error
 */
const writeQueueFullError = (label, buffered, limit) => () => {
  const error = new Error(`hot-shots: dropped metric, the ${label} socket already has ${buffered} bytes ` +
    `waiting to flush (limit ${limit})`);
  error.code = WRITE_QUEUE_FULL_CODE;
  return error;
};

/**
 * Marks a resolver error as a failed DNS lookup, so Client.close() can tell it
 * apart from a write failure. The resolver's own code stays on `code`, so an
 * errorHandler matching ENOTFOUND or EAI_AGAIN keeps matching.
 * @param {Error|null} error - the lookup error, or null on success
 * @returns {Error|null} the same error
 */
const markLookupFailed = (error) => {
  if (error && typeof error === 'object' && !error.hotShotsCode) {
    error.hotShotsCode = DNS_LOOKUP_FAILED_CODE;
  }
  return error;
};

/**
 * The pending-byte cap for a transport. A missing, non-numeric or non-positive
 * option falls back to the default rather than disabling the cap or, worse,
 * refusing every send.
 * @param {Object} args - transport args
 * @returns {number} the cap in bytes
 */
const pendingWriteLimit = (args) => {
  const configured = Number(args.maxPendingWriteBytes);
  return Number.isFinite(configured) && configured > 0 ? configured : MAX_PENDING_WRITE_BYTES;
};

/**
 * Builds the gate that decides when a socket over its pending-byte cap has
 * genuinely stopped draining, as opposed to merely not having yielded yet.
 *
 * writableLength only falls when control returns to the event loop, so a
 * synchronous emit loop accumulates its whole burst before a single byte is
 * flushed - no matter how fast the peer is reading. Refusing on the raw byte
 * count therefore drops metrics a healthy agent would have taken. The
 * discriminator is an event-loop turn: a synchronous burst is by definition one
 * tick and never reaches the refusal, while a peer that has actually stopped
 * reading is caught on the very next turn. Progress since the last check
 * restarts the turn, so a slow-but-draining consumer is never refused either.
 * @param {number} limit - the pending-byte cap
 * @returns {Object} shouldRefuse(buffered) and cancel()
 */
const createWriteStallGate = (limit) => {
  let pendingTurn = null;
  let settled = false;
  let high = 0;

  /**
   * Starts a fresh event-loop turn before refusals may begin.
   * @returns {void}
   */
  const armTurn = () => {
    settled = false;
    // Restart rather than reuse an armed turn. Reusing it would let a turn that
    // began before the most recent progress settle the gate, refusing a peer
    // that is draining, only slowly.
    if (pendingTurn) {
      clearImmediate(pendingTurn);
    }
  ...
  /**
   * Clears any armed turn, so a closed transport leaves nothing scheduled.
   * @returns {void}
   */
  const cancel = () => {
    if (pendingTurn) {
  ...
  return {
    shouldRefuse: (buffered) => {
      if (buffered <= limit) {
        cancel();
        return false;
      }
      if (high === 0 || buffered < high) {
        // First time over the cap, or the socket drained since the last check.
        // Either way the peer is making progress, so start the turn over.
        high = buffered;
        armTurn();
        return false;
      }
...
const createTcpTransport = args => {
  ...
  const writeLimit = pendingWriteLimit(args);
  const stallGate = createWriteStallGate(writeLimit);
  return {
    ...
    send: (buf, callback) => {
      ...
      // metric into retained memory. Refuse once too much has been waiting
      // across an event-loop turn without draining. See createWriteStallGate for
      // why the byte count alone is not enough.
      if (stallGate.shouldRefuse(socket.writableLength)) {
        debug('hot-shots createTcpTransport: %d bytes already unflushed, dropping metric', socket.writableLength);
        failLater(callback, writeQueueFullError('tcp', socket.writableLength, writeLimit));
        return;
      }
...
const createUdpTransport = args => {
  ...
  if (!socketOptions.lookup) {
    ...
    socketOptions.lookup = (hostname, options, callback) => {
      ...
      // Tagged so a per-packet lookup failure does not abort Client.close().
      dns.lookup(hostname, options, (err, address, family) => {
        callback(markLookupFailed(err), address, family);
      });
    };
  } else {
    // A caller-supplied lookup is kept, with its failures tagged the same way.
    const callerLookup = socketOptions.lookup;
    socketOptions.lookup = (hostname, options, callback) => {
      if (typeof options === 'function') {
        const optionsCallback = options;
        callerLookup(hostname, (err, address, family) => {
          optionsCallback(markLookupFailed(err), address, family);
        });
        return;
      }
      callerLookup(hostname, options, (err, address, family) => {
        callback(markLookupFailed(err), address, family);
      });
    };
  }
   ...
   * @returns {void}
   */
  const recordLookupFailure = (error) => {
    dnsResolutionData.failureTimestamp = Date.now();
    dnsResolutionData.consecutiveFailures++;
    // Kept so a send refused during the cooldown can carry the resolver's own
    // code. An errorHandler matching ENOTFOUND or EAI_AGAIN would otherwise stop
    // matching after the first failure of a streak.
    dnsResolutionData.lastError = error || null;
  };
   ...
   * consecutive failure and is capped at DNS_COOLDOWN_MAX_MS.
   *
   ...
   *
   * The ceiling is a few seconds rather than cacheDnsTtl because nothing
   * schedules a retry: only an incoming send re-attempts the lookup. The
   * cooldown is therefore also the time a client keeps refusing sends after the
   * resolver is healthy again, and a TTL-sized ceiling made that up to a minute.
   * @returns {number} the cooldown in milliseconds
   */
  const cooldownMs = () => Math.min(DNS_COOLDOWN_MAX_MS,
  ...
  const startLookup = () => {
    ...
    try {
      ...
      dns.lookup(args.host, { family: family }, (error, address) => {
        ...
        if (error) {
          ...
          if (isRefresh) {
            ...
            flushPending(markLookupFailed(error));
          }
          return;
        }
        ...
        dnsResolutionData.lastError = null;
      ...
      dnsResolutionData.refreshInFlight = false;
      recordLookupFailure(lookupError);
      if (isRefresh) {
        ...
        setImmediate(() => flushPending(markLookupFailed(lookupError)));
      }
    }
  };
  ...
  const sendUsingDnsCache = (callback, buf) => {
    ...
    if (!dnsResolutionData.refreshInFlight && isCoolingDown()) {
      ...
      // Read now rather than inside the factory below, which runs a tick later:
      // the code reported must be the one that was current when the send was
      // refused, not whatever a lookup in between happened to leave behind.
      const resolverCode = dnsResolutionData.lastError && dnsResolutionData.lastError.code;
      failLater(callback, () => {
        ...
        // The resolver's own code stays on `code` so existing handlers keep
        // matching through a failure streak; the cooldown marker travels
        // alongside it. Client.handleCallback reads hotShotsCode first when
        // routing telemetry and deciding whether close() may continue.
        error.code = resolverCode || DNS_COOLDOWN_CODE;
        error.hotShotsCode = DNS_COOLDOWN_CODE;
        return error;
      });
      return;
    }
  ...
  return {
    ...
    cancelPendingSends: (error) => {
      ...
      // One flush is enough: the latch above is set before it runs, and
      // sendUsingDnsCache checks that latch before every push, so a callback
      // that resends cannot put anything back into the queue.
      flushPending(error);
    },
...
const createUdsTransport = args => {
  ...
  const sendWithRetry = (buf, callback, attempt = 0) => {
    ...
    // unix-dgram invokes this callback synchronously, inside socket.send(). A
    // throw from the user's callback therefore lands in the catch below, after
    // the send has already been called back. Tracked so the catch does not
    ...
    try {
      socket.send(buf, (err) => {
        ...
        if (err && isRetryableUdsError(err) && attempt < maxRetries) {
          ...
          // Set first, then trim, matching the DNS pending queue. Trimming
          // first would let a resending drop callback grow the map by one per
          // send. Map iteration is insertion-ordered, so the first key is the
          // oldest pending retry.
          while (pendingRetries.size > UDS_MAX_PENDING_RETRIES) {
            const oldest = pendingRetries.keys().next().value;
            const oldestCallback = pendingRetries.get(oldest);
            pendingRetries.delete(oldest);
            clearTimeout(oldest);
            debug('hot-shots UDS transport: %d retries already pending, dropping the oldest',
              UDS_MAX_PENDING_RETRIES);
            failLater(oldestCallback, () => {
              const dropErr = new Error(
                `hot-shots: dropped metric, ${UDS_MAX_PENDING_RETRIES} uds sends are already waiting out a retry backoff`);
              dropErr.code = UDS_RETRY_QUEUE_FULL_CODE;
              return dropErr;
            });
          }
        } else if (err) {
          debug('hot-shots UDS transport: send error - %s (attempts: %d)', err.message || err.code || err, attempt + 1);
          // Deferred: unix-dgram calls back on send()'s own stack frame, so a
          // resending errorHandler would otherwise recurse. See failLater.
          failLater(typeof callback === 'function' ? callback : null, () => err);
        } else {
      ...
      if (calledBack) {
        // The send was already called back, and this throw came from that
        // callback. Calling back again would break the exactly-once contract
        // the client's drain logic depends on.
        console.error(`hot-shots: a uds send callback threw: ${socketError && socketError.message}`);
        return;
      }
...
const createStreamTransport = (args) => {
  ...
  return {
    ...
    send: (buf, callback) => {
      ...
      if (stallGate.shouldRefuse(stream.writableLength)) {
        debug('hot-shots stream transport: %d bytes already unflushed, dropping metric', stream.writableLength);
        failLater(callback, writeQueueFullError('stream', stream.writableLength, writeLimit));
        return;
      }
...
module.exports = (instance, args) => {
  ...
  try {
    ...
    if (instance.errorHandler) {
      // Contained: callers reach this from a socket 'error' emit during a
      // replacement, where a throwing handler would end the process.
      callErrorHandler(instance, e, 'transport creation');
    } else {
      console.error(e);
    }
  }
