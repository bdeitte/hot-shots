...
const { DNS_CANCELLED_CODE, DNS_MAX_PENDING, DNS_QUEUE_FULL_CODE, PROTOCOL } = require('./constants');
...
const createUdpTransport = args => {
  ...
  const sendUsingDnsCache = (callback, buf) => {
    ...
    // this lookup), refuse to queue: nothing will ever cancel or flush a
    // second generation of entries, so failing fast here is what keeps every
    // callback to exactly one invocation. See the `cancelled` flag comment
      ...
      if (callback) {
        setImmediate(() => {
          const error = new Error(
            `hot-shots: send rejected, DNS resolution for ${args.host} was cancelled during close`);
  ...
  return {
    ...
    // messagesInFlight, once this transport has latched cancelled. True only
    // when this specific send would otherwise hit the cacheDns cold-start
    // queueing path - a warm resolved address, an IP literal, or a
    // non-cacheDns send are unaffected, matching what socket.close() already
    // leaves alone. UDP-only; callers must check for its presence.
    isDnsSendBlocked: () => dnsResolutionData.cancelled &&
      Boolean(args.host) && net.isIP(args.host) === 0 && dnsResolutionData.resolvedAddress === undefined,
    ...
    close: () => {
      ...
      // Also latch here (redundant with cancelPendingSends when Client.close()
      // ran first) so a transport closed directly still refuses to re-queue
      // sends from callbacks fired by the flushPending call below.
      dnsResolutionData.cancelled = true;
      if (dnsResolutionData.pending.length > 0) {
