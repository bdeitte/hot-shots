...
/**
 * Calls a client's errorHandler, containing a throw so it cannot escape. Most
 * callers reach this from an event emit, a timer, or a transport's I/O
 * callback, where nothing above would catch it and the process would end. The
 * synchronous callers use it so one handler bug cannot surface at an unrelated
 * call site. Callers must check that client.errorHandler is set.
