...
function overrideTags (parent, child, telegraf) {
  ...
  // Fast path: empty child array/object means no overrides. Return a shallow copy of
  // parent — the slow path returns a new array (filter + push), so callers may rely on
  // not receiving the same array reference back.
