...
function isHostCgroupNamespace(statSync) {
  try {
    const stat = statSync(OD.CGROUP_NS_PATH); // eslint-disable-line no-sync
...
function readContainerIDFromCgroup(readFileSync) {
  try {
    return matchContainerID(readFileSync(OD.CGROUP_PATH)); // eslint-disable-line no-sync
  } catch (e) {
...
function readContainerIDFromMountInfo(readFileSync) {
  try {
    return matchContainerID(readFileSync(OD.MOUNTINFO_PATH)); // eslint-disable-line no-sync
  } catch (e) {
...
function getCgroupInode(readFileSync, statSync) {
  let content;
  try {
    content = readFileSync(OD.CGROUP_PATH); // eslint-disable-line no-sync
  } catch (e) {
  ...
  for (const line of lines) {
    ...
    if (parts.length >= 3) {
      ...
      if (controllerSegment !== undefined) {
        ...
        try {
          const stat = statSync(fullPath); // eslint-disable-line no-sync
          if (stat && stat.ino > 2) {
            return `in-${stat.ino}`;
          }
        } catch (e) {
