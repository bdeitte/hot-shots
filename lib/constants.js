...
// Marks the error given to sends that were already queued behind an in-flight
// DNS lookup and got cancelled because close() gave up waiting on it. This is
// the ONLY meaning of this code - it must stay scoped to genuine cancellation.
// A send that merely arrives after the client is already closed is a
// different case: see DNS_CLOSED_CODE below. Client.close()'s final-flush
// error branch treats this code and DNS_CLOSED_CODE the same way (report and
// keep closing rather than aborting) - a repeat close() on an already-closed
// cacheDns client rejects its final flush with DNS_CLOSED_CODE, not this one,
// and that must still let close() finish. The two codes stay distinct for
// consumers; only close()'s own branch treats them alike.
exports.DNS_CANCELLED_CODE = 'HOTSHOTS_DNS_CANCELLED';

// Marks the error given to a send that arrives after Client.close() has
// already closed this transport's cacheDns queue - as opposed to
// DNS_CANCELLED_CODE, which is for a send that was already queued and got
// cancelled mid-lookup. Paired with dnsQueueClosedMessage(host) below, used at
// the two send-after-close sites (Client.prototype.sendMessage's
// isDnsSendBlocked branch, and sendUsingDnsCache's `cancelled` branch).
