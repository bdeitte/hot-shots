...
// flat full-TTL cooldown would turn a one-second resolver blip at startup into a
// full TTL of dropped metrics, while still being no better at preventing thrash
// than a short first wait that doubles.
...
// abandoned rather than fired against a closing socket; the analogue of
// DNS_CANCELLED_CODE for the retry queue.
exports.UDS_RETRY_CANCELLED_CODE = 'HOTSHOTS_UDS_RETRY_CANCELLED';

// Maximum bytes a connection-oriented transport (tcp, stream) may leave sitting
...
// so without this a stalled connect turns every metric into retained memory.
...
// Codes meaning "the client refused this send outright; nothing reached the
...
// The final flush timed out and close() gave up on it. Not a refusal - the send
// may yet complete - but close() must still proceed rather than hang forever.
...
// How long (ms) close() waits for the final buffered flush before giving up on
// it and proceeding to drain and close anyway. Much larger than the drain budget
// at the default closingFlushInterval (50 * 11, ~550ms) on purpose: a first-ever
// DNS lookup or a TCP connect taking a few hundred ms is ordinary and must not
// lose the flush, so this only bounds a genuinely stuck transport. A caller who
// raises closingFlushInterval past ~455 inverts that ordering; the two budgets
// still bound their own phases, close() just reaches this one first.
