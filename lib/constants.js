...
// A lookup that actually failed, as opposed to a send hot-shots refused. Set on
// hotShotsCode, with the resolver's own code left on `code`. Not a refusal, so
// it is absent from REFUSED_CODES and still counts as a writer drop. It is in
// CLOSE_CONTINUE_CODES: a resolver outage leaves close() nothing to wait for,
// and aborting would leave the socket open.
exports.DNS_LOOKUP_FAILED_CODE = 'HOTSHOTS_DNS_LOOKUP_FAILED';

// How long (ms) the first failed cacheDns lookup waits before another attempt.
// Each further consecutive failure doubles the wait, capped at
// DNS_COOLDOWN_MAX_MS. The ramp exists because the cold path has no cached
// address to fall back on. A flat full-TTL cooldown would turn a one-second
// resolver failure at startup into a full TTL of dropped metrics. It would also
// be no better at preventing thrash than a short first wait that doubles.
exports.DNS_COOLDOWN_BASE_MS = 1000;

// The ceiling the doubling stops at. Only an incoming send retries the lookup,
// so the cooldown is also how long a client keeps refusing sends after the
// resolver is healthy again. Capping at cacheDnsTtl made that up to 60s by
// default. Seconds bound the recovery lag while keeping the property that
// matters: never one lookup per send.
exports.DNS_COOLDOWN_MAX_MS = 5000;
...
// Codes meaning "the client gave up on this send itself", rather than a write
// that reached the socket and failed. Counted as queue drops rather than writer
// errors. Most are a refusal with no I/O attempted at all. The two uds retry
// codes are the exception: that send did reach the socket and came back EAGAIN,
// and what was abandoned is the retry, not the first attempt. A failure the
// transport genuinely reported for a write or a lookup does not belong here.
...
// send was refused outright, close() stopped waiting on it, or its DNS lookup
// failed. In each case the socket still needs closing and the caller's callback
// still needs to fire.
exports.CLOSE_CONTINUE_CODES = exports.REFUSED_CODES.concat([
  exports.CLOSE_FLUSH_TIMEOUT_CODE,
  exports.DNS_LOOKUP_FAILED_CODE
]);
