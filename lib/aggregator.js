...
function contextKey(client, name, type, tags, cardinality) {
  ...
  if (tags === undefined || tags === null) {
    ...
    // emitted form) alongside its type so values that format as different tags do
    // not collide. Passing values straight to JSON.stringify would collapse an
    // array-position `undefined`, `NaN`, `Infinity` and `-Infinity` all to `null`,
    // merging contexts that emit as distinct tags (`a:undefined` vs `a:null`,
    // `a:NaN` vs `a:Infinity` vs `a:-Infinity`).
