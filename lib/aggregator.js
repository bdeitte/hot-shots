...
const METRIC_TYPES = require('./constants').METRIC_TYPES;
...
const Aggregator = function (options) {
  ...
  this.closed = false;
  debug('hot-shots aggregator initialized: flushInterval=%d, maxContexts=%d',
    this.flushInterval, this.maxContexts);
};
...
function clientContextSuffix(client) {
  if (client._aggContextSuffix === undefined) {
    // Canonicalize global tags to their emitted form (the same way per-call tags
    // are keyed) so child clients sharing an aggregator do not split contexts for
    // global tags that differ only in a way that disappears on the wire — e.g.
    // Telegraf `route:a` vs `route=a`, which both emit as `route=a`.
    client._aggContextSuffix =
      `${canonicalTagsKey(client, client.globalTags)}|${client.containerID || ''}|${client.externalData || ''}`;
  }
  return client._aggContextSuffix;
}

/**
 * Serializes per-call tags into the canonical form they will actually be emitted
 * as, so two tag inputs that reach the wire identically map to one context. Keys
 * on the emitted form via the same `formatTags` path `send()` uses, plus
 * Telegraf's `key:value`→`key=value` rewrite, then order-normalizes. This folds
 * together inputs that differ only in a way that disappears on the wire:
 *   - array vs object form (`['a:1']` and `{a: 1}` both emit `a:1`)
 *   - tag order (the server treats tag sets as unordered)
 *   - Telegraf `a:1` vs `a=1` (both emit `a=1`)
 * Keying on the raw caller form instead would let two contexts that emit as
 * identical tags flush in creation rather than update order, settling a gauge on
 * a stale value. Value stringification is handled by `sanitizeTagValue`'s
 * `String()`, so `undefined`/`null` and `NaN`/`Infinity`/`-Infinity` stay
 * distinct (they emit as distinct tags) rather than collapsing as they would
 * through a bare `JSON.stringify`.
 * @param client The client the metric was recorded through.
 * @param tags {Array|Object=} Per-call tags, exactly as passed by the caller.
 * @returns {String} The canonical tag key.
 */
function canonicalTagsKey(client, tags) {
  if (tags === undefined || tags === null) {
    return '';
  }
  // Non-array, non-object tags are emitted as-is; key on the raw value.
  if (typeof tags !== 'object') {
    return JSON.stringify(tags);
  }
  let formatted = helpers.formatTags(tags, client.telegraf);
  if (client.telegraf) {
    // Mirror send()'s telegraf rewrite: the first colon becomes the key/value
    // separator `=`, so `a:1` and `a=1` both emit — and now key — as `a=1`.
    formatted = formatted.map(tag => {
  ...
  // Sort a copy so tag sets that differ only in order map to one context.
  // Emission keeps caller order via cloneTags — only the key is normalized.
  return JSON.stringify(formatted.slice().sort());
}
...
function contextKey(client, name, type, tags, cardinality) {
  const tagsKey = canonicalTagsKey(client, tags);
...
Aggregator.prototype.record = function (client, name, value, type, tags, cardinality) {
  ...
  if (!context) {
    ...
    if (this.contexts.size >= this.maxContexts) {
      debug('hot-shots aggregator record: context cap reached (%d), rejecting - name=%s, type=%s',
        this.maxContexts, name, type);
      this.signalOverflow(client);
      return false;
    }
    debug('hot-shots aggregator record: new context - name=%s, type=%s', name, type);
    context = {
      client: client,
      name: name,
      type: type,
      tags: helpers.cloneTags(tags),
      cardinality: cardinality,
      value: type === METRIC_TYPES.SET ? new Set() : 0,
    };
  ...
  debug('hot-shots aggregator record: aggregated - name=%s, type=%s, value=%s', name, type, value);
  return true;
};
...
Aggregator.prototype.sendContext = function (context) {
  try {
    if (context.type === METRIC_TYPES.SET) {
      for (const value of context.value) {
        context.client.send(`${context.name}:${value}|${METRIC_TYPES.SET}`, context.tags, context.cardinality);
      }
    } else {
...
Aggregator.prototype.flushContext = function (client, name, type, tags, cardinality) {
  ...
  debug('hot-shots aggregator flushContext: sending pending context early - name=%s, type=%s', name, type);
  try {
