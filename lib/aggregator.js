...
const Aggregator = function (options) {
  ...
  // Every client a flush has ever routed a metric through. close() drains these
  // before destroying the shared socket so an interval-driven flush (which passes
  // no involvedClients set) can't leave a child's send in flight. See close().
  this.routedClients = new Set();
...
function contextKey(client, name, type, tags, cardinality) {
  ...
  if (tags === undefined || tags === null) {
    ...
    // could deliver a stale final value. This also stops `{a: undefined}` and
    // `{}` from colliding into the same context.
    tagsKey = JSON.stringify(Object.keys(tags).sort().map(k => [k, tags[k]]));
  } else {
    tagsKey = JSON.stringify(tags);
  }
...
Aggregator.prototype.flush = function (involvedClients) {
  ...
  for (const context of contexts.values()) {
    ...
    // Always record the client so close() can drain it, even for interval-driven
    // flushes that pass no involvedClients set.
    this.routedClients.add(context.client);
    if (context.type === 's') {
