...
function applyStatsFns (Client) {
  ...
  Client.prototype.timer = function (func, stat, sampleRate, tags, callback) {
    ...
    return (...args) => {
      ...
      try {
        ...
        const finalTags = mergeTags(tags, ctx.getTags());
        _this.timing(
          stat,
          duration,
          sampleRate,
   ...
   */
  function mergeTags(originalTags, dynamicTags) {
    ...
    // Convert originalTags to array if it's an object
    let tagsArray;
    if (Array.isArray(originalTags)) {
      tagsArray = originalTags.slice();
    } else if (typeof originalTags === 'object') {
      tagsArray = Object.keys(originalTags).map(key => `${key}:${originalTags[key]}`);
    } else {
      tagsArray = [];
    }
