...
function applyStatsFns (Client) {
  ...
  Client.prototype.timing = function (stat, time, sampleRate, tags, callback) {
    ...
    this.sendAll(stat, t, METRIC_TYPES.TIMING, sampleRate, tags, callback);
  };

  /**
   * Represents the timing stat by recording the duration a function takes to run (in milliseconds)
   * @param func {Function} The function to run. Receives a context object as the last argument
   *   with an `addTags(tags)` method to add tags dynamically during execution.
   * @param stat {String|Array} The stat(s) to send
   * @param sampleRate {Number=} The Number of times to sample (0 to 1). Optional.
   * @param tags {Array=} The Array of tags to add to metrics. Optional.
   * @param callback {Function=} Callback when message is done being delivered. Optional.
   ...
   *
   * @param func {<T,A>(...A):Promise<T>} The function to run. Receives a context object as the last
   *   argument with an `addTags(tags)` method to add tags dynamically during execution.
   * @param stat {String|Array} The stat(s) to send
   * @param sampleRate {Number=} The Number of times to sample (0 to 1). Optional.
   * @param tags {Array=} The Array of tags to add to metrics. Optional.
   * @param callback {Function=} Callback when message is done being delivered. Optional.
  ...
  Client.prototype.asyncTimer = function (func, stat, sampleRate, tags, callback) {
    ...
    return (...args) => {
      const ctx = createTimerContext();
      const end = hrtimer();
      const p = func(...args, ctx);
      const recordStat = () => {
        const finalTags = mergeTags(tags, ctx.getTags(), self.telegraf);
        self.timing(stat, end(), sampleRate, finalTags, callback);
      };
  ...
  Client.prototype.asyncDistTimer = function (func, stat, sampleRate, tags, callback) {
    ...
    return (...args) => {
      ...
      const recordStat = () => {
        const finalTags = mergeTags(tags, ctx.getTags(), self.telegraf);
        self.distribution(stat, end(), sampleRate, finalTags, callback);
      };
      p.then(recordStat, recordStat);
      return p;
    };
  ...
  /**
   * Creates a timer context object that allows adding tags during function execution
   */
  function createTimerContext() {
    ...
    return {
       ...
       * @param tags {Object|Array} Tags to add. Can be an object like {key: 'value'} or array like ['key:value']
       */
      addTags: function (tags) {
        ...
        if (Array.isArray(tags)) {
          ...
          // Convert object tags to array format
          Object.keys(tags).forEach(key => {
            dynamicTags.push(`${key}:${tags[key]}`);
          });
        }
      },
      /**
       * Get all accumulated tags
       */
      getTags: function () {
        return dynamicTags;
      }
  ...
  /**
   * Merge original tags with dynamically added tags.
   * Uses helpers.formatTags to ensure consistent tag formatting.
   */
  function mergeTags(originalTags, dynamicTags, telegraf) {
    ...
    // Use the canonical formatTags helper for consistent formatting
    const tagsArray = helpers.formatTags(originalTags, telegraf);
    return tagsArray.concat(dynamicTags);
  }
  ...
  Client.prototype.increment = function (stat, value, sampleRate, tags, callback) {
    ...
    if (typeof value !== 'number') {
      // Shift parameters if: less than 3 args, OR sampleRate looks like a callback
      // and tags wasn't explicitly provided (issue #139)
      if (arguments.length < 3 || (typeof sampleRate === 'function' && tags === undefined)) {
        callback = sampleRate;
        tags = value;
    ...
    this.sendAll(stat, value, METRIC_TYPES.COUNT, sampleRate, tags, callback);
  };
  ...
  Client.prototype.histogram = function (stat, value, sampleRate, tags, callback) {
    ...
    this.sendAll(stat, value, METRIC_TYPES.HISTOGRAM, sampleRate, tags, callback);
  };
  ...
  Client.prototype.distribution = function (stat, value, sampleRate, tags, callback) {
    ...
    this.sendAll(stat, value, METRIC_TYPES.DISTRIBUTION, sampleRate, tags, callback);
  };
  ...
  Client.prototype.gauge = function (stat, value, sampleRate, tags, callback) {
    ...
    this.sendAll(stat, value, METRIC_TYPES.GAUGE, sampleRate, tags, callback);
  };
   ...
   */
    Client.prototype.gaugeDelta = function (stat, value, sampleRate, tags, callback) {
      ...
      this.sendAll(stat, `${sign}${Math.abs(value)}`, METRIC_TYPES.GAUGE, sampleRate, tags, callback);
    };
  ...
  Client.prototype.unique = Client.prototype.set = function (stat, value, sampleRate, tags, callback) {
    ...
    this.sendAll(stat, value, METRIC_TYPES.SET, sampleRate, tags, callback);
  };
   ...
   *   @option cardinality {String} Tag cardinality for this check (DogStatsD datadog mode only).
   ...
   *   @option cardinality {String} Tag cardinality for this event (DogStatsD datadog mode only).
   * @param tags {Array=} The Array of tags to add to metrics. Optional.
   * @param callback {Function=} Callback when message is done being delivered. Optional.
   */
  Client.prototype.event = function (title, text, options, tags, callback) {
    ...
    // Apply prefix and suffix to the title, similar to how check() does it
    const rawTitle = String(title ? title : '');
    const msgTitle = this.prefix + rawTitle + this.suffix;
    let msgText = String(text ? text : rawTitle);
    // Escape new lines (unescaping is supported by DataDog)
    msgText = msgText.replace(/\n/g, '\\n');

    // start out the message with the event-specific title and text info.
    // The DogStatsD event header carries TITLE_UTF8_LENGTH and TEXT_UTF8_LENGTH,
    // which are byte counts of the UTF-8 encoding, so use Buffer.byteLength
    // rather than String#length (UTF-16 code units).
    message = `_e{${Buffer.byteLength(msgTitle)},${Buffer.byteLength(msgText)}}:${msgTitle}|${msgText}`;
