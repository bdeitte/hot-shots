...
* `tcpEncoding`: Used only when the protocol is `tcp`. The encoding the socket is written with. `default: 'ascii'`, which is node's latin1 encoder and so keeps only the low byte of each utf-16 code unit, corrupting any non-ascii metric name, tag or event field. Set it to `'utf8'` to send non-ascii intact, as the `udp`, `uds` and `stream` transports already do. Payloads that are entirely ascii are byte-identical either way. Note that the DogStatsD event header (`_e{...}`) always counts utf-8 bytes, per the datagram format, so events with non-ascii titles or text need `'utf8'` here to be self-consistent over `tcp`.
